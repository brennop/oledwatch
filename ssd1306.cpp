...
void SSD1306Device::setInvert(bool value){
	invert = value;
}

void SSD1306Device::blank(uint8_t width)
{
	ssd1306_send_data_start();
...
size_t SSD1306Device::write(byte c) {
	...
	if(oledFont == SMALL){
		...
		{
			data = pgm_read_byte(&smallfont[ci * SMALL_WIDTH + i]);
			if(invert) data = ~data;
			ssd1306_send_data_byte(data);
		}
		ssd1306_send_data_stop();
		setCursor(oledX+SMALL_WIDTH, oledY);
	}
	else{
		for(uint8_t j = 0; j < BIG_HEIGHT / 8; j++){
			ssd1306_send_data_start();
			for (i = 0; i < BIG_WIDTH; i++)
			{
				data = pgm_read_byte(&bigfont[ci * BIG_WIDTH * (BIG_HEIGHT/8) + i + j * BIG_WIDTH]);
				if(invert) data = ~data;
				ssd1306_send_data_byte(data);
			}
			ssd1306_send_data_stop();
		...
		setCursor(oledX + BIG_WIDTH, oledY - 2); 
	}
	return 1;
}
